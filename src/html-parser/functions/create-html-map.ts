...
import { dynamicToModelToValue } from './create-map/dynamic-to-model-to-value';
import { wrappedToModelToString } from './create-map/wrapped-to-model-to-string';
import { attributeToProperty } from './parser/attribute-to-property';
import { getWrappedDynamicValueParts } from './create-map/get-wrapped-dynamic-value-parts';
import { wrappedToModelToValue } from './create-map/wrapped-to-model-to-value-map';
...
export function createHtmlMap(maps: ValueMapDescriptor[]): (d: HtmlElementTemplateDescriptor) => ElementTemplateDescriptor {
  ...
  const sMap = partial(wrappedToModelToString, getParts);
  const vMap = partial(wrappedToModelToValue, getParts);
