...
export function createDataObservable(
  model: Value,
  state: State,
  data: Dict<Data>
): Observable<Dict<Value>> {
  const observables: Array<Observable<any>> = dictToArray(data).map(
    (dataItem: Data) => {
    ...
    return forkJoin(...observables).pipe(
      map((values: Value[]) => {
        return values.reduce(
          (result: Dict<Value>, item: Value, currentIndex: number) => {
