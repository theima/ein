...
export function dynamicPartToModelToValue(
  maps: Dict<ValueMap>,
  dynamicStringValue: DynamicPart
): ModelToValue {
  return (model: Value) => {
    const parts: string[] = trimStrings(
      dynamicStringValue.split(ParseString.MapSeparator)
    );
    const value: NullableValue | undefined = getModel(
      model,
    ...
    return parts.reduce((value: Value, part: string) => {
      const mapAndParameters: string[] = trimStrings(
        part.split(ParseString.ParameterSeparator)
      );
      ...
      const parameters = mapAndParameters.slice(1).map((param) => {
        ...
        if (result === null || result === undefined) {
          throw new Error(
            "Could not parse parameter'" + param + "' for '" + mapName + "'"
          );
        }
        return result;
      });
